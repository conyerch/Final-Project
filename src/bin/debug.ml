...
let debugInfo(ouch, str, term) =
  (if !debug then
     debugOut(ouch, Ast1.ppStream, str, term)
   else ())

let debug2Info(ouch, str, term) =
  (if !debug then
     debugOut(ouch, Ast2.ppStream, str, term)
   else ())

let debug3Info(ouch, str, term) =
  (if !debug then
     debugOut(ouch, Ast3.ppStream, str, term)
   else ())
