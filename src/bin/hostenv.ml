(*
 * Adapted from spectest in wasm reference, this lets us hijack the print to a string
 *)

open Wasm
open Types
open Values
open Instance


let global (GlobalType (t, _) as gt) =
  let v =
    match t with
    | I32Type -> I32 666l
    | I64Type -> I64 666L
    | F32Type -> F32 (F32.of_float 666.6)
    | F64Type -> F64 (F64.of_float 666.6)
  in Global.alloc gt v

let table = Table.alloc (TableType ({min = 10l; max = Some 20l}, FuncRefType))
let memory = Memory.alloc (MemoryType {min = 1l; max = Some 2l})
let func f t = Func.alloc_host t (f t)

let print_value buf v =
  Printf.bprintf buf "%s : %s\n"
    (Values.string_of_value v) (Types.string_of_value_type (Values.type_of v))

let print strref (FuncType (_, out)) vs =
  List.iter (print_value strref) vs;
  flush_all ();
  List.map default_value out


let lookup strref name t =
  match Utf8.encode name, t with
  | "print", _ -> ExternFunc (func (print strref) (FuncType ([], [])))
  | "print_i32", _ -> ExternFunc (func (print strref) (FuncType ([I32Type], [])))
  | _ -> raise Not_found
