...
let testEquivProgram (p1 : Ast.program) (p2 : Ast.program) =
  ...
  and
    ...
    | App c1, App c2 ->
      if testEquivId m c1.rator c2.rator then
        List.iter (fun (t1, t2) -> if testEquivId m t1 t2 then () else raise Not_equiv)
          (List.combine c1.rands c2.rands)
      else
        raise Not_equiv
    | _, _ -> raise Not_equiv
