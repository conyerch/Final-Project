...
let operatorCodeGenerators =
  ...
  *)
  ...
  (function
    | [rd; rs; rt] ->
      fIL [toInstr(None, Opn.And {rd; rs = rt; rt = rs}, Some "and")]
    | _ -> failwith "Dynamicbasis: bad call of the and codegen.");
  (* or *)
  (function
    | [rd; rs; rt] ->
      fIL [toInstr(None, Opn.Or {rd; rs = rt; rt = rs}, Some "and")]
    | _ -> failwith "Dynamicbasis: bad call of the or codegen.");
]
